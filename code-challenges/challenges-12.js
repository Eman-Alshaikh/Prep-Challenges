...
const cvsFiltration = (arr) => arr.filter(candidate => candidate.yearsOfExperience>4 && candidate.tech === 'JS'&& candidate.firstName !== null && candidate.LastName !== null).map(candidate => {
    candidate["fullName"]  = `${candidate.firstName} ${candidate.LastName}`;
    delete candidate.firstName;
    delete candidate.LastName;
    delete candidate.yearsOfExperience;
    return candidate;
});
